...
async function askOmnibox() {
    ...
    // ══════════════════════════════════════════════════════
    // שאלת חומרה עוקפת הכל — ישר לגמיני
    var HW_RX = /פוד|משאבה|קנולה|חיישן|דקסקום|dexcom|להחליף|להמתין|כשל|סתום|תקוע|מחט|תקין|לא עובד/i;
    if (HW_RX.test(q)) { askGeminiAdvisor(q); return; }
...
function buildGeminiSystemPrompt(cr, isf, ctx) {
    ...
    // זיהוי שאלת חומרה — לפני בניית הפרומפט
    var _q = (ctx && ctx._userQ) ? ctx._userQ : '';
    var _isHardware = /פוד|משאבה|קנולה|חיישן|דקסקום|dexcom|להחליף|להמתין|כשל|סתום|תקוע|מחט|תקין|לא עובד/i.test(_q);
    var cage = (ctx && ctx.cage != null) ? Math.round(ctx.cage) : null;

    if (_isHardware) {
        return "אתה עוזר טיפולי לסוכרת סוג 1. המשתמש שאל שאלת חומרה (פוד/משאבה/חיישן).\n" +
            "חובה לענות! לעולם אל תסרב, אל תכתוב 'איני יכול לספק ייעוץ', ואל תכתוב 'לא צוין מאכל'.\n" +
            "זו לא שאלת אוכל — אל תשתמש בפורמט מאכל כלל!\n\n" +
            "נתוני אמת חיים מ-NS:\n" +
            "סוכר = " + sgv + " " + trend + " | IOB = " + iob + "U | COB = " + cob + "g | בזאלי = " + basal + "U/ש'\n" +
            (cage != null ? "גיל פוד (CAGE) = " + cage + " שעות\n" : "") +
            (p30 ? "תחזית: בעוד 30דק'=" + p30 + " | eventual=" + pEv + "\n" : "") + "\n" +
            "חוקי אבחון:\n" +
            "• SGV>240 + IOB>1.5U + גרף שטוח/עולה למרות בזאלי = כשל קנולה/פוד מת:\n" +
            "  '🚨 כשל ספיגה! החלף פוד פיזית במיקום חדש + בולוס תיקון במזרק/עט חיצוני — לא דרך הלופ!'\n" +
            "• CAGE<12 שעות + תיקון אחרון מוצלח = פוד חדש:\n" +
            "  'הפוד חדש (" + (cage||'?') + " שעות). אל תחליף — תן לאינסולין 1.5 שעות להתניע, שב על הידיים.'\n" +
            "• CAGE>40 שעות = ציין כגורם לתנגודת, שקול החלפה.\n" +
            "• נתונים תקינים = 'הפוד תקין, אין סימני כשל. המתן ובדוק שוב בעוד שעה.'\n\n" +
            "ענה בעברית, קצר וברור, עם אבחון + פעולה. השתמש בנתונים האמיתיים למעלה.";
    }
